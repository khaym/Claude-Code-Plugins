...
#   - cleanup fires on session end (on-session-end.sh deletes exactly the
#     ending session's file); the age fallback removes only 31+-day leftovers
#     — a week-old queue of a live resident session survives
#
...
rm -rf "$DIR"
ctx=$(echo "{\"session_id\":\"$SID_A\"}" | bash "$SESSION_START")
