...
| `pipx` denied; `uvx` / `uv tool install` not denied (D-1) | `pipx run` and `pipx install` bypass `[tool.uv]` settings (`exclude-newer`, `index-strategy`). The sibling `hardening-uv-config` skill ships `uvx <pkg>@<version>` and `uv tool install <pkg>` as the sanctioned replacements. Mirrors the `npx` deny rationale |
| `pip install` intentionally not denied (D-1) | Persistent installer threats are addressed at the config layer (`[tool.uv]` cooldown, `uv.lock` integrity), not the permission layer. Mirrors the stance on `npm install`. Users who want stricter enforcement may extend D-1 with `Bash(pip install *)` themselves |
...
| D-1 | `Bash` deny | Block outbound (`curl`/`wget`/`nc`), dynamic execution (`eval`), privilege escalation (`sudo`/`su`), and unsafe package execution (`npx`, `pipx`) | 15 patterns |
