...
| Stage 4 names no implementer agent (as of v0.1.0) | The bundled `implementer` agent arrives with the loop skill (the plugin's loop-generalization stage); a name with no bundled referent would dangle. Stage 4 carries the generic delegation guidance and gets extended in the version that bundles the agent. |
| Host bindings name no verify/observe skills (as of v0.1.0) | The `verify` / `observe` project-skill convention ships with the loop skill; until that version, domain quality nets bind through the host CLAUDE.md alone. Extend the bindings table's home column in the version that defines the convention. |
