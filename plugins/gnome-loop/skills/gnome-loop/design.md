...
| Human-facing replies come after ScheduleWakeup | Only a turn's final text — with no tool call after it — renders reliably in the terminal; a reply written before the pacing call can drop, and a dropped report reads as a silent stop (observed 2026-08-22, the plugin's first host equivalence run). Ordering is free to fix — the pacing call does not have to be last — and the ticket log stays the durable copy. |
