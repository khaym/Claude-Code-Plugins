...
| A parent ticket takes no loop transition (since 0.5.0) | A parent is a container — no implementation, no lap of its own — and its children are the lap units. Its rules are prose a human applies at the grant and at the close, not a net (dev-cycle's Swiss Cheese: a net is inserted after a failure class is observed): (1) granting loop-ready is a human act, so the human reading the conditions is the point of control; (2) a parent granted by mistake surfaces at the human-owned merge approval and costs at most one lap. A pick-time exclusion would be a net; it is deferred until a mistakenly granted parent is observed reaching a lap. The rule lives here, not in dev-cycle, because it is loop vocabulary (statuses, grant, pick, merge); in a dialog lap the parent's close follows from ticket-authoring (a parent's Done is its success criteria) and Human Audit alone. |
