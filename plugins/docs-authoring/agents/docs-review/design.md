...
| `model: inherit` | Principle application needs context understanding (P0 viewpoint, P3 substitution test). Inheriting the parent model avoids locking the agent to a tier that may be wrong for the caller's task. Revisit if cost becomes an issue. |
