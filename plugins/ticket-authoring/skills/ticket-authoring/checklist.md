...
| T3 | **Done shape and viewpoint** — Done opens with one prose sentence describing a behavior or state change visible to the user; each `- [ ]` item below it is a success criterion — a verifiable end-state in the outcome's vocabulary, independent of the other items and not a restatement of the opening sentence; implementation details (file, function, test names) appear only as parenthetical references, never as items |
