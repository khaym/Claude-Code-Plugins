...
**仕組み:**
- **単位は 1 つ** — 1 チケット = 利用者が得る 1 つの能力。実装ステップは単独チケットにせず、Done はそれが仕える成功条件を持つ
- **5 つの前提チェック（T1–T5）** — 判別テスト / 目的の係留 / Done の形と視点 / 判断依頼の語彙 / 境界と依存（起票時のセルフチェックと ticket-review エージェントで共用）
...
- キューのファイルはセッションごとに 1 つ: 並行セッションの項目は混ざらない。hook がセッション開始時と毎プロンプトの context 注入で自セッションのファイルパスを Claude に伝える
- 自己修復する登録: hook がセッション開始のたびに安定パス `~/.claude/decision-queue/statusline.sh` をインストール済みバージョンへ張り直すため、plugin 更新後も次のセッション開始で登録が復元される
- キューのファイルはセッション終了時と resume 時に削除され、resume 後のセッションは空から始まる。どのセッションにも resume されなかった残骸は 30 日で回収

**前提:**
- `jq` が PATH にあること

**セットアップ（初回のみ）:** ユーザー設定ファイル（`~/.claude/settings.json`）への追記は 2 つ——キューのディレクトリへの書き込みを Claude に許可するルールと、statusline の登録。plugin はどちらも同梱できないため、一度だけ手動で足します。`statusLine` が持てるコマンドは 1 つなので、ラッパーで両方を呼ばない限り、このレンダラが既存の statusline を置き換えます。手順とラッパーの書き方は [plugin の README](plugins/decision-queue/README.md#setup-one-time)（英語）にあります。
